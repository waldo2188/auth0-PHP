...
class ApiTests extends \PHPUnit_Framework_TestCase
{
     ...
     */
    protected static function getApi($endpoint, array $actions, $returnType = null)
    {
        self::getEnv();
        ...
        if (! empty( $fields[0] )) {
            ...
            foreach ($data as $index => $datum) {
                $data[$index] = $fields[1] ?
                    ...
                    array_intersect_key( $datum, $check_fields ) :
                    // Remove the keys indicated.
                    array_diff_key( $datum, $check_fields );
            }
        }
     ...
     */
    protected function assertUri($request, $path = '')
    {
        $this->assertEquals(
